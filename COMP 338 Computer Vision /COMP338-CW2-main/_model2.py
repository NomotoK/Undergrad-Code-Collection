...
class BasicUnit(nn.Module):
    def __init__(self, channels: int, dropout: float):
        ...
        self.block = nn.Sequential(OrderedDict([
            ...
            ("1_activation", nn.ReLU()),  # Removed inplace=True
            ("2_convolution", nn.Conv2d(channels, channels, (3, 3), stride=1, padding=1, bias=False)),
            ("3_normalization", nn.BatchNorm2d(channels)),
            ("4_activation", nn.ReLU()),  # Removed inplace=True
            ("5_dropout", nn.Dropout(dropout)),  # Removed inplace=True
...
class DownsampleUnit(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int, dropout: float):
        ...
        self.block = nn.Sequential(OrderedDict([
            ...
            ("2_activation", nn.ReLU()),  # Removed inplace=True
            ("3_dropout", nn.Dropout(dropout)),  # Removed inplace=True
...
class WideResNet(nn.Module):
    def __init__(self, depth: int, width_factor: int, dropout: float, in_channels: int, labels: int):
        ...
        self.f = nn.Sequential(OrderedDict([
            ...
            ("5_activation", nn.ReLU()),  # Removed inplace=True
            ("6_pooling", nn.AvgPool2d(kernel_size=1)),  # Changed kernel_size to 1
            ("7_flattening", nn.Flatten()),
            ("8_classification", nn.Linear(in_features=self.filters[3] * 1 * 1, out_features=labels)),
        ]))
...
model = WideResNet(depth=28, width_factor=10, dropout=0.3, in_channels=1, labels=10)